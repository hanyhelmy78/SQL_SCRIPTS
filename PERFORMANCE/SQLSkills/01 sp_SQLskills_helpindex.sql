...
IF OBJECTPROPERTY(OBJECT_ID(N'sp_SQLskills_helpindex'), N'IsProcedure') = 1
...
AS
--November 2021: Cleaned up and consolidated so only one helpindex for usage AND for finddupes. Default is "unordered" which means "as created." For dupes, we 'order' the INCLUDE list so that we essentially normalize these lists.
