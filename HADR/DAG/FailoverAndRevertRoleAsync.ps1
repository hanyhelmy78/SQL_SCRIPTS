...
#########Alter DAG, change the current Primary AG to as a seconadary role #########
#########Failover#########
#########Revert back the old secondary to Async commit mode#########
...
for ($in = 0; $in -lt $ag_table.count; $in++)
{
    $PrimaryAgName = ($ag_table[$in] | select AGName).AGName
    ...
    $Go_or_NoGoQuery = "SET NOCOUNT ON; select SecondaryLName
    from (
    SELECT  replace(substring(endpoint_url,1,charindex('.',endpoint_url)-1),'TCP://','') SecondaryLName
    ...
    and dag.name = '$DACName')a
    where AG_Role = 2"

    $AG_status = sqlcmd -S $ServerInstanceWithPort -d $database -E -Q $Go_or_NoGoQuery
    $SecondaryLis = $AG_status | where {$_ -notlike "*---*" -and $_ -notlike "*SecondaryLName*"}
    ...
    $AgDacPrimaryToSecondaryQuery = "ALTER AVAILABILITY GROUP [$DACName] SET (ROLE = SECONDARY);"
    $AG_status = sqlcmd -S $ServerInstanceWithPort -d $database -E -Q $AgDacPrimaryToSecondaryQuery
    $AG_status

    if ([string]::IsNullOrEmpty($AG_status)) 
    {
        Write-Host "The Distributed AG " -ForegroundColor Green -NoNewline
        Write-Host $DACName -ForegroundColor Yellow -NoNewline
        Write-Host " on " -ForegroundColor Green -NoNewline
        Write-Host $PrimaryReplica -ForegroundColor Cyan -NoNewline
        Write-Host " replica has been " -ForegroundColor Green -NoNewline
        Write-Host "changed to Secondary role." -ForegroundColor Green -BackgroundColor black
    }

    $ServerInstanceWithPort = $SecondaryLis.Trim()+","+$PrimaryLPort.Trim()

    $AgDacSecondaryReplicaQuery = "SET NOCOUNT ON; SELECT @@servername SecondaryReplicaName;"
    $AG_status = sqlcmd -S $ServerInstanceWithPort -d $database -E -Q $AgDacSecondaryReplicaQuery
    $SecondaryReplica = $AG_status | where {$_ -notlike "*---*" -and $_ -notlike "*SecondaryReplicaName*"}
    $SecondaryReplica = $SecondaryReplica.Trim()
 
    $ServerInstanceWithPort = $SecondaryLis.Trim()+","+$PrimaryLPort.Trim()
    $AgDacFailoverQuery = "ALTER AVAILABILITY GROUP [$DACName] FORCE_FAILOVER_ALLOW_DATA_LOSS;"
    $AG_status = sqlcmd -S $ServerInstanceWithPort -d $database -E -Q $AgDacFailoverQuery
    $AG_status
    if ([string]::IsNullOrEmpty($AG_status)) 
    {
        Write-Host "The Distributed AG " -ForegroundColor Green -NoNewline
        Write-Host $DACName" " -ForegroundColor Yellow -NoNewline
        Write-Host "has successfully failed over" -ForegroundColor Green -BackgroundColor Black -NoNewline
        Write-Host " to the replica " -ForegroundColor Green -NoNewline
        Write-Host $SecondaryReplica -ForegroundColor Cyan -NoNewline
        Write-Host "." -ForegroundColor Green 
    }

    $ServerInstanceWithPort = $PrimaryLName.Trim()+","+$PrimaryLPort.Trim()
    $AsyncSecondaryAgQuery = "ALTER AVAILABILITY GROUP [$DACName]
    MODIFY AVAILABILITY GROUP ON '$SecondaryAG'
    WITH (AVAILABILITY_MODE = ASYNCHRONOUS_COMMIT);"
    $AG_status = sqlcmd -S $ServerInstanceWithPort -d $database -E -Q $AsyncSecondaryAgQuery
    $AG_status

    if ([string]::IsNullOrEmpty($AG_status)) 
    {
        Write-Host "The old Secondary AG " -ForegroundColor Green -NoNewline
        Write-Host $SecondaryAG  -ForegroundColor Yellow -NoNewline
        Write-Host ", now the Primary of Distributed AG " -ForegroundColor Green -NoNewline
        Write-Host $DACName -ForegroundColor Yellow -NoNewline
        Write-Host " has been successfully set to " -ForegroundColor Green -NoNewline
        Write-Host "ASYNCHRONOUS_COMMIT" -ForegroundColor Cyan  -BackgroundColor Black -NoNewline
