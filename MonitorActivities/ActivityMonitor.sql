...
FROM sys.dm_exec_sessions s WITH (NOLOCK) LEFT OUTER JOIN sys.dm_exec_connections c WITH (NOLOCK) ON (s.session_id = c.session_id)
...
LEFT OUTER JOIN 
(
-- In some cases (e.g. parallel queries, also waiting for a worker), one thread can be flagged as waiting for several different threads.  
-- This will cause that thread to show up in multiple rows in our grid, which we don't want.  
-- Use ROW_NUMBER to select the longest wait for each thread, and use it as representative of the other wait relationships this thread is involved in. 
...
WHERE case when p.dbid=0 then N'' else ISNULL(db_name(p.dbid),N'') end = 'TAKHIFAD'
