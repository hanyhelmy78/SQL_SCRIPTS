...
--Full Backup------------------------------------------------------------------------------------
			...
			set @BackupType='Full Backup'
				set @sql = @sql + N' 
						...
						BACKUP DATABASE [' + @name + '] TO  [' + @name + '_Tape] WITH NOFORMAT, INIT,  NAME = N''' + @name + '-Full Database Backup'', SKIP, NOREWIND, NOUNLOAD, COMPRESSION, STATS = 5'			
				END
--------------------------------------------------------------------------------------------------
--Diffrential Backup------------------------------------------------------------------------------------
			IF (@DiffBackup = 1)
			...
			set @BackupType='Differentical Backup'
						...
						BACKUP DATABASE [' + @name + '] TO  [' + @name + '_Tape] WITH DIFFERENTIAL , NOFORMAT, INIT,  NAME = N''' + @name + '-Differential Database Backup'', SKIP, NOREWIND, NOUNLOAD, COMPRESSION, STATS = 5'				
				END
--------------------------------------------------------------------------------------------------
--TXN Log overwirte-------------------------------------------------------------------------------
...
if (@count > 0 )
				...
				BACKUP LOG [' + @name + '] TO  [' + @name + '] WITH NOFORMAT, ' + @TXLLOGTypeName + ',  NAME = N''' + @name + '-Transaction Log Backup'', SKIP, NOREWIND, NOUNLOAD, COMPRESSION, STATS = 5'
				end
					else 
				begin
