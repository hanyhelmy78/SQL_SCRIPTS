...
/* SQL Server is really awesome in that it doesn't do any sort of re-ordering of the filter definition when it's created. It does reformat the definition adding parens and brackets, but I'm not sure how it determines the order of the predicates (original? different?). That said..Filters can only use 'AND' statements, they cannot use 'OR'. So that means we can split, re-order and concat the filters to create a more reliable string for matching.
